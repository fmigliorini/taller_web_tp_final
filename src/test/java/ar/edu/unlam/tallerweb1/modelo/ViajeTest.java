...
public class ViajeTest extends SpringTest {
	...
	@Transactional
	@Rollback(true)
	public void listarViajesTerminados() {
		...
		List<Viaje> listarViajesActivos = session.createCriteria(Viaje.class).add(Restrictions.eq("estado", "terminado"))
				.createAlias("vehiculo", "veh").createAlias("veh.chofer", "ch").add(Restrictions.eq("ch.rol", "chofer"))
				.list();

		assertThat(listarViajesActivos).hasSize(2);
		
		for(Viaje viajesActivos :listarViajesActivos ){
			assertEquals("chofer",viajesActivos.getVehiculo().getChofer().getRol());
			assertEquals("terminado",viajesActivos.getEstado());
