						...
						<tbody>
							<c:forEach var="mov" items="${presupuestos}">
								<tr>
									...
									<td><fmt:formatNumber pattern="0000" type="number"
											value="${mov.getPuntoVenta()}" /> - <fmt:formatNumber
											pattern="00000000" type="number"
											value="${mov.getNumeroMovimiento()}" /></td>
									<td><c:out value="${mov.getUsuario().getNombre()}" /></td>
									<td><c:out
											value="${mov.getViaje().getVehiculo().getChofer().getNombre()}" /></td>
